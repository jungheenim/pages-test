...
switch($fileExt){
	...
	default:
		echo "Nur (jpg, bmp, gif, png) File ist möglich hochzuladen.";
		exit;
		break;
}
...
if($fileType == 'image'){
	...
	if($extStatus){
		...
		// 업로드 성공 여부 확인
		if($imageUpload == true){
			echo "File ist erfolgreich hochgeladen. <br>";
			echo "<img src='{$resFile}' width='100' />";
		}else{
			echo "Nicht erfolgreich hochgeladen.";
		}
	}	// end if - extStatus
		// 확장자가 jpg, bmp, gif, png가 아닌 경우 else문 실행
	else {
		echo "File muss jpg, bmp, gif, png sein.";
		exit;
	}
}	// end if - filetype
	// 파일 타입이 image가 아닌 경우
else {
	echo "Das ist kein Imagefile.";
	exit;
}
?>
